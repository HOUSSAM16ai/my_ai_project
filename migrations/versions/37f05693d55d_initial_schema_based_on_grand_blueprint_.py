"""Initial schema based on Grand Blueprint v10.2

Revision ID: 37f05693d55d
Revises: 
Create Date: 2025-08-24 12:11:13.263924

"""
from alembic import op
import sqlalchemy as sa
# --- [THE CRITICAL FIX - STEP 1] ---
# We import our custom type directly, making it available to the script.
from app.models import JSONB_or_JSON


# revision identifiers, used by Alembic.
revision = '37f05693d55d'
down_revision = None
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    # We create the tables in the correct dependency order.
    # Tables without foreign keys, or that are parents, come first.
    ...
    # Add the foreign key from missions to mission_plans now that both tables exist
    with op.batch_alter_table('missions', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_missions_active_plan_id', 'mission_plans', ['active_plan_id'], ['id'])
...
def downgrade():
    ...
    # (The downgrade part is usually correct, but should also be reviewed)
    with op.batch_alter_table('submissions', schema=None) as batch_op:
    ...
    with op.batch_alter_table('missions', schema=None) as batch_op:
        # Drop the explicitly created foreign key first
