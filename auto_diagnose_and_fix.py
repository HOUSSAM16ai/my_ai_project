...
class CogniForgeAutoDiagnostic:
    ...
    def check_dependencies(self) -> DiagnosticResult:
        ...
        try:
            ...
            required_packages = ["flask", "sqlalchemy", "openai", "requests"]
    ...
    def fix_migrations(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(
                    f"{Colors.FAIL}   ❌ Failed to apply migrations: {result.stderr}{Colors.ENDC}"
                )
                return False
        ...
        if failed_critical:
            ...
            for result in failed_critical:
                ...
                if "API keys" in result.check_name:
                    ...
                    print("     3. Add: OPENROUTER_API_KEY=sk-or-v1-your-key-here")
                    print("     4. Save and restart application")
...
def main():
    ...
    critical_failures = sum(
        1 for r in diagnostic.results if not r.passed and r.severity == "critical"
    )
