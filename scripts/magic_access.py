...
def kill_process_on_port(port):
    print(f"{Colors.WARNING}⚠️  Port {port} is occupied. Scanning for unauthorized entities...{Colors.ENDC}")
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            for conn in proc.connections(kind='inet'):
                if conn.laddr.port == port:
                    print(f"{Colors.FAIL}🔥 DETECTED HOSTILE: {proc.info['name']} (PID: {proc.info['pid']}){Colors.ENDC}")
                    proc.terminate()
                    proc.wait(timeout=3)
...
def main():
    ...
    if is_port_in_use(PORT_BACKEND):
        print(f"{Colors.WARNING}⚡ Port {PORT_BACKEND} is active. Ensuring system integrity...{Colors.ENDC}")
        ...
        time.sleep(1)
        if is_port_in_use(PORT_BACKEND):
             print(f"{Colors.FAIL}❌ Failed to clear port {PORT_BACKEND}. Manual intervention required.{Colors.ENDC}")
             sys.exit(1)
