...
if grep -q "your-openrouter-api-key" "$PROJECT_ROOT/.env"; then
...
echo "  3. Run migrations: ${BLUE}docker-compose run --rm web python -m cli db-migrate${NC}"
echo "  4. Seed database (creates admin user): ${BLUE}docker-compose run --rm web python -m cli db seed --confirm${NC}"
echo ""
