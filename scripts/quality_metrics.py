...
def get_code_metrics() -> dict[str, Any]:
    ...
    # Count lines of code
    code, output = run_command("find app/ -name '*.py' -not -path '*/migrations/*' -exec wc -l {} + | tail -1")
    if code == 0:
        try:
    ...
    if code == 0:
        try:
            ...
            metrics['average_complexity'] = round(total_complexity / function_count, 2) if function_count > 0 else 0
...
def get_security_metrics() -> dict[str, Any]:
    ...
    try:
        ...
        metrics['security_total'] = metrics['security_high'] + metrics['security_medium'] + metrics['security_low']
    except:
...
def display_dashboard(metrics: dict[str, Any]) -> None:
    ...
    score_color = Colors.GREEN if overall_score >= 80 else Colors.YELLOW if overall_score >= 60 else Colors.RED
    ...
    coverage_color = Colors.GREEN if coverage >= 80 else Colors.YELLOW if coverage >= 30 else Colors.RED
    ...
    print(f"  Lines Covered:          {test.get('lines_covered', 0):,}/{test.get('lines_total', 0):,}")
    ...
    print(f"  {Colors.YELLOW}Medium Severity:        {security.get('security_medium', 0)}{Colors.NC}")
    ...
    pylint_color = Colors.GREEN if pylint_score >= 8.0 else Colors.YELLOW if pylint_score >= 6.0 else Colors.RED
