        ...
        if 'except Exception as e:' in route_code:
            ...
            if ', 500' in route_code and 'jsonify' in route_code:
                ...
                if ', 500' in exception_section:
                    print(f"   ❌ Still returns 500 status code!")
            ...
            if ', 200' in route_code or '), 200' in route_code:
                print(f"   ✅ Returns 200 with error details")
            else:
                print(f"   ⚠️  Warning: No explicit 200 status found")
            
            # Check for bilingual error messages
            if '⚠️' in route_code or 'خطأ' in route_code or 'فشل' in route_code:
                print(f"   ✅ Has bilingual error messages")
            else:
                print(f"   ⚠️  No bilingual error messages found")
            
            # Check for answer field in error response
            if '"answer"' in route_code:
                print(f"   ✅ Returns 'answer' field for frontend")
            else:
                print(f"   ⚠️  No 'answer' field in error response")
        else:
            print(f"   ⚠️  No exception handler found")
