...
def test_admin_conversation_system():
    ...
    with app.app_context():
        ...
        if not admin:
            admin = User(full_name="Test Admin", email="admin@test.com", is_admin=True)
        ...
        conversation = service.create_conversation(
            user=admin, title="Test SUPERHUMAN Conversation System", conversation_type="testing"
        )
        ...
        print(
            f"   Avg Response Time: {conversation.avg_response_time_ms:.2f}ms"
            if conversation.avg_response_time_ms
        ...
        if analytics.get("status") == "success":
            ...
            print(
                f"   Avg Response Time: {analytics['avg_response_time_ms']:.2f}ms"
        ...
        for msg in messages:
            ...
            print(
                f"   Content Hash: {msg.content_hash[:16]}..."
                if msg.content_hash
                else "   Content Hash: N/A"
            )
        ...
        if success:
            ...
            print("❌ Failed to archive conversation")
