...
def check_error_handling_coverage():
    ...
    if os.path.exists(service_file):
        ...
        checks.append(('Service: Try-catch in _build_super_system_prompt', 
                      'try:' in service_code and '_build_super_system_prompt' in service_code))
        checks.append(('Service: Fallback prompt on error', 
                      'fallback' in service_code.lower()))
        checks.append(('Service: Size limits on project index', 
                      'max_index_size' in service_code))
        checks.append(('Service: Size limits on file content', 
                      'max_file_size' in service_code))
        checks.append(('Service: Logging prompt size', 
    ...
    if os.path.exists(routes_file):
        ...
        checks.append(('Routes: Question length validation', 
                      'max_question_length' in routes_code))
        checks.append(('Routes: Top-level try-catch', 
                      'try:' in routes_code and 'handle_chat' in routes_code))
        checks.append(('Routes: Conversation creation error handling', 
                      'try:' in routes_code and 'create_conversation' in routes_code))
        checks.append(('Routes: Detailed error messages', 
