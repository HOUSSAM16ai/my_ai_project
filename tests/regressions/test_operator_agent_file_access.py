...
def test_should_skip_task_allow_read_file_with_metadata_only(operator_agent):
    ...
    """
    context = MockContext(
        data={"exercise_metadata": {"year": 2024, "subject": "Math"}},
        shared_memory={}
    )

    should_skip, reason = operator_agent._should_skip_task("read_file", context)
    assert should_skip is False, f"Should not skip read_file when only metadata is present. Reason: {reason}"

    should_skip, reason = operator_agent._should_skip_task("list_dir", context)
    assert should_skip is False, f"Should not skip list_dir when only metadata is present. Reason: {reason}"
...
def test_should_skip_task_block_shell_always_with_context(operator_agent):
    ...
    """
    context = MockContext(
        data={"exercise_metadata": {"year": 2024}},
        shared_memory={}
    )
