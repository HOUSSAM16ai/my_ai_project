...
def test_flexible_enum_resilience():
    ...
    # 1. Test valid value
    m1 = TestModel(status=TestStatus.ACTIVE)
    session.add(m1)
    session.commit()

    m1_loaded = session.query(TestModel).filter_by(id=m1.id).first()
    assert m1_loaded.status == TestStatus.ACTIVE
    assert isinstance(m1_loaded.status, TestStatus)
    ...
    m2 = TestModel(status="UNKNOWN_VALUE")
    session.add(m2)
    session.commit()
    ...
    m2_loaded = session.query(TestModel).filter_by(id=m2.id).first()

    # It should be the string "unknown_value"
    assert m2_loaded.status == "unknown_value"
    assert not isinstance(m2_loaded.status, TestStatus)
