...
from app.api.routers.overmind import router as overmind_router, get_orchestrator
from app.api.routers.customer_chat import router as customer_router, get_db
from app.api.routers.ws_auth import extract_websocket_auth, _parse_protocol_header, _extract_token_from_protocols
...
def test_create_mission_error(overmind_app):
    ...
    response = client.post("/api/v1/overmind/missions", json={"objective": "valid objective length"})
...
def test_customer_ws_decode_fail(customer_app):
    client = TestClient(customer_app)
    with patch("app.api.routers.customer_chat.extract_websocket_auth", return_value=("token", "jwt")):
...
def test_customer_ws_admin(customer_app):
    client = TestClient(customer_app)
    mock_user = MagicMock(spec=User); mock_user.is_active = True; mock_user.is_admin = True
    mock_db = AsyncMock(); mock_db.get.return_value = mock_user
...
def test_customer_ws_empty_question(customer_app):
    client = TestClient(customer_app)
    mock_user = MagicMock(spec=User); mock_user.is_active = True; mock_user.is_admin = False
    mock_db = AsyncMock(); mock_db.get.return_value = mock_user
    customer_app.dependency_overrides[get_db] = lambda: mock_db
...
def test_extract_token_from_protocols():
    assert _extract_token_from_protocols(["jwt"]) == None
    assert _extract_token_from_protocols(["other"]) == None
