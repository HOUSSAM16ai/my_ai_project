...
from app.api.routers.admin import router, get_db, get_current_user_id, get_ai_client, get_chat_dispatcher, get_session_factory
...
def test_chat_stream_ws_orchestrator_error(app):
    ...
    with patch("app.api.routers.admin.extract_websocket_auth", return_value=("valid_token", "json")):
        with patch("app.api.routers.admin.decode_user_id", return_value=1):
            with patch("app.services.chat.orchestrator.ChatOrchestrator.dispatch", side_effect=HTTPException(status_code=400, detail="Orchestrator error")):
                with client.websocket_connect("/admin/api/chat/ws") as websocket:
