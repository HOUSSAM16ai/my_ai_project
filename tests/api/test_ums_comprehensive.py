...
from app.api.routers.ums import get_auth_service, router
...
def test_create_user_admin_with_reauth(client, mock_auth_service, mock_admin_user):
    ...
    new_user.id = 102
    new_user.email = "new@t.com"
    new_user.full_name = "New"
    ...
    payload = {"full_name": "New Admin", "email": "new@t.com", "password": "pass", "is_admin": True}
...
def test_assign_role_admin_reauth_fail(client, mock_auth_service, mock_admin_user):
    ...
    response = client.post(
        "/admin/users/101/roles", json={"role_name": ADMIN_ROLE, "justification": "Because"}
    )
...
def test_assign_role_justification_too_short(client, mock_auth_service, mock_admin_user):
    ...
    response = client.post(
        "/admin/users/101/roles", json=payload, headers={"X-Reauth-Password": "pass"}
    )
