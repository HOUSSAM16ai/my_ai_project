...
from app.api.routers.observability import router, get_observability_service
...
def mock_service():
    ...
    service.get_performance_snapshot.return_value = {"cpu_usage": 20.5, "memory_usage": 40.2, "active_requests": 5}
    service.get_endpoint_analytics.return_value = [{
        "path": "/test", "avg_latency": 15.0, "p95_latency": 25.0, "error_count": 0, "total_calls": 10
    }]
    service.get_active_alerts.return_value = [{
        "id": "alert1", "severity": "low", "message": "test", "timestamp": "now", "status": "active"
    }]
    return service
