...
from app.api.routers.crud import router, get_crud_service
from app.api.schemas.crud import GenericResourceResponse, PaginatedResponse
from app.schemas.management import PaginatedResponse as BoundaryPaginatedResponse, PaginationMeta
...
def test_list_resources_success(client, mock_service):
    ...
    boundary_result = BoundaryPaginatedResponse(
        items=items,
        pagination=PaginationMeta(total_items=2, page=1, per_page=10, total_pages=1, has_next=False, has_prev=False)
    )
