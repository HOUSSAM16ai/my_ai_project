...
from app.core.database import engine, validate_and_fix_schema
...
async def test_pragma_table_info_with_valid_table():
    ...
    async with engine.begin() as conn:
        ...
        if dialect_name == "sqlite":
            ...
            await conn.execute(
                text("CREATE TEMPORARY TABLE IF NOT EXISTS test_table (id INTEGER, name TEXT)")
            )

            # Test the new parameterized approach
            result = await conn.execute(
                text("SELECT * FROM pragma_table_info(:table_name)"), {"table_name": "test_table"}
            )
...
async def test_no_sql_injection_via_table_name():
    ...
    async with engine.begin() as conn:
        ...
        if dialect_name == "sqlite":
            ...
            await conn.execute(text("CREATE TEMPORARY TABLE IF NOT EXISTS test_users (id INTEGER)"))
