...
def test_execute_chaos_experiment_success():
    ...
    with patch.object(service, '_inject_failure') as mock_inject, \
...
def test_calculate_resilience_score_with_history():
    ...
    execution1 = ChaosExecution(
        execution_id="1", schedule_id=None, scenario=FailureScenario.SERVICE_CRASH,
        started_at=datetime.now(UTC), passed=True, system_recovered=True, recovery_time_seconds=60.0
    )
    execution2 = ChaosExecution(
        execution_id="2", schedule_id=None, scenario=FailureScenario.SERVICE_CRASH,
        started_at=datetime.now(UTC), passed=False, system_recovered=False, recovery_time_seconds=0.0
    )
