...
from app.services.chat_orchestrator_service import ChatOrchestratorService, ChatIntent, IntentResult
...
async def test_handle_file_read_flow(orchestrator):
    ...
    with patch("app.services.chat_orchestrator_service.CircuitBreakerRegistry.get", return_value=mock_cb):
...
async def test_handle_code_search_flow(orchestrator):
    ...
    orchestrator._async_tools.code_search_lexical.return_value = {
        "ok": True,
        "data": {"results": [{"file": "models.py", "line": 10, "match_line_excerpt": "class User:"}]}
    }
