    ...
    with (
        ...
        client.nodes_map[primary_model] = NeuralNode(
            ...
            circuit_breaker=CircuitBreaker(
                "Primary", CIRCUIT_FAILURE_THRESHOLD, CIRCUIT_RECOVERY_TIMEOUT
            ),
        )
        client.nodes_map[backup_model] = NeuralNode(
            model_id=backup_model,
            circuit_breaker=CircuitBreaker(
                "Backup", CIRCUIT_FAILURE_THRESHOLD, CIRCUIT_RECOVERY_TIMEOUT
            ),
        )
        ...
        with (
            patch("app.core.ai_gateway.ConnectionManager.get_client", return_value=mock_client),
            ...
            primary_calls = [
                c for c in calls if c.kwargs.get("json", {}).get("model") == primary_model
            ]
            ...
            assert len(primary_calls) == 0, (
                f"Primary model should have been skipped, but was called {len(primary_calls)} times"
            )

            # Verify backup was used
            backup_calls = [
                c for c in calls if c.kwargs.get("json", {}).get("model") == backup_model
            ]
