...
REQUIRED_SCHEMA: dict[str, TableSchema] = {'admin_conversations':
    ...
    ColumnType.INTEGER, nullable=False), ColumnDefinition('title',
    ColumnType.VARCHAR, nullable=False), ColumnDefinition('user_id',
    ColumnType.INTEGER, nullable=False, index=True), ColumnDefinition(
    'conversation_type', ColumnType.VARCHAR, nullable=True),
    ColumnDefinition('linked_mission_id', ColumnType.INTEGER, nullable=True,
    index=True), ColumnDefinition('created_at', ColumnType.TIMESTAMP,
    nullable=True)]), 'admin_messages': TableSchema(name='admin_messages',
    columns=[ColumnDefinition('id', ColumnType.INTEGER, nullable=False),
    ColumnDefinition('conversation_id', ColumnType.INTEGER, nullable=False,
...
class SQLGenerator:
    ...
    def add_column(table: str, column: ColumnDefinition) ->str:
        ...
        sql = (
            f'ALTER TABLE "{table}" ADD COLUMN IF NOT EXISTS "{col_name}" {column.type.value}'
            )
    ...
    def create_index(table: str, column: str) ->str:
        ...
        return (
            f'CREATE INDEX IF NOT EXISTS "{index_name}" ON "{table}"("{col_name}")'
            )
...
class SelfHealingEngine:
    ...
    async def _core_healing_logic(self, strategy: ExecutionStrategy,
        auto_fix: bool) ->None:
    ...
    async def _apply_fix(self, strategy: ExecutionStrategy, table: str,
        column: ColumnDefinition):
        op = HealingOperation(table=table, column=column.name, operation=
            'add_column', sql=SQLGenerator.add_column(table, column))
        try:
    ...
    def _apply_fix_sync(self, strategy: ExecutionStrategy, table: str,
        column: ColumnDefinition):
        op = HealingOperation(table=table, column=column.name, operation=
            'add_column', sql=SQLGenerator.add_column(table, column))
        try:
    ...
    def _finalize_report(self):
        ...
        self._report.status = ('success' if not self._report.errors else
            'partial')
        self._log_report()
