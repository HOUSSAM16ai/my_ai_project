...
async def test_check_search_providers_no_egress():
    ...
    with patch("app.services.overmind.readiness.ProviderReadinessGate._check_egress_detailed", new_callable=AsyncMock) as mock_egress:
...
async def test_check_mission_readiness_delegates():
    ...
    with patch("app.services.overmind.readiness.ProviderReadinessGate.check_search_providers", new_callable=AsyncMock) as mock_check:
