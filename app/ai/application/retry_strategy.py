...
class RetryConfig:
    ...
    retry_on_categories: set[ErrorCategory] = field(default_factory=lambda :
        {ErrorCategory.RATE_LIMIT, ErrorCategory.NETWORK, ErrorCategory.
        SERVER_ERROR, ErrorCategory.TIMEOUT})
...
class RetryStrategy(ABC):
    ...
    def record_attempt(self, category: ErrorCategory, success: bool) ->None:
        ...
        if self.metrics.total_attempts > 0:
            self.metrics.average_attempts = self.metrics.total_attempts / (
                self.metrics.successful_retries + self.metrics.
                failed_retries + 1)
...
class RetryExecutor:
    ...
    async def execute_async(self, func: Callable[..., Any], *args: Any, **
        kwargs: Any) ->Any:
        """
