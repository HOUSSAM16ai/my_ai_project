...
class CircuitBreakerConfig:
    ...
    def calculate_timeout(self, failure_count: int) ->float:
        ...
        base_timeout = min(self.timeout * self.exponential_backoff_base **
            failure_count, self.max_timeout)
...
class CircuitBreaker:
    ...
    async def call_async(self, func: Callable[..., Any], *args: Any, **
        kwargs: Any) ->Any:
        """
    ...
    def _on_success(self) ->None:
        ...
        with self._lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                if (self.metrics.consecutive_successes >= self.config.
                    success_threshold):
    ...
    def _on_failure(self, error: Exception) ->None:
        ...
        with self._lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                ...
                if (self.metrics.consecutive_failures >= self.config.
                    failure_threshold):
                    self._transition_to_open()
...
class CircuitBreakerRegistry:
    ...
    def get_or_create(self, name: str, config: (CircuitBreakerConfig | None
        )=None) ->CircuitBreaker:
    ...
    def get_all_metrics(self) ->dict[str, CircuitMetrics]:
        ...
        return {name: breaker.get_metrics() for name, breaker in self.
            _breakers.items()}
