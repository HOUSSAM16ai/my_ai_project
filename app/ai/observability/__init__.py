# app/ai/observability/__init__.py
"""
...
class TraceLevel(Enum):
    ...
    STANDARD = "standard"  # Standard tracing
    DETAILED = "detailed"  # Full request/response
    ...
    def get_all_metrics(self) -> dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "timers": {
                    k: self.get_timing_stats(k.split("[")[0])
...
class ObservabilityManager:
    ...
    def trace_operation(
        ...
        **metadata: Any
        ...
        self.metrics.increment("traces.started", tags={"operation": operation})
    ...
    def finish_operation(
        ...
        **metadata: Any
        ...
        self.metrics.increment("traces.finished", tags={
        ...
        for hook in self._hooks:
            try:
                ...
                _LOG.warning(f"Span hook error: {e}")
    
    def get_dashboard_data(self) -> dict[str, Any]:
        """Get data for monitoring dashboard."""
        recent_spans = [
            s for s in self.tracer.get_all_spans()
            if s.end_time and time.time() - s.end_time < 3600
        ]
        
        return {
            "metrics": self.metrics.get_all_metrics(),
            "active_traces": len([s for s in recent_spans if s.status == "in_progress"]),
            "completed_traces": len([s for s in recent_spans if s.status != "in_progress"]),
            "error_rate": self._calculate_error_rate(recent_spans),
            "avg_duration_ms": self._calculate_avg_duration(recent_spans),
            "operations": self._get_operation_stats(recent_spans),
        }
