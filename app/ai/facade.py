...
class LLMClientService:
    ...
    **REFACTORED**: This class now delegates to specialized services
    ...
    def __init__(self, provider: (str | None)=None, circuit_breaker_config:
        (CircuitBreakerConfig | None)=None, retry_config: (RetryConfig |
        None)=None):
        """
        ...
        self.response_normalizer = ResponseNormalizer(cost_manager=self.
            cost_manager)
        self.circuit_breaker = get_circuit_breaker(
            f'llm_client_{self.provider}', circuit_breaker_config)
        retry_strategy = AdaptiveRetry(retry_config
            ) if retry_config else AdaptiveRetry()
    ...
    def _initialize_default_budgets(self) ->None:
        ...
        self.cost_manager.set_budget('global_daily', BudgetConfig(limit=
            daily_limit, period=BudgetPeriod.DAILY, soft_limit_percentage=
            0.8, hard_limit_percentage=1.0, auto_reset=True))
        self.cost_manager.set_budget('global_monthly', BudgetConfig(limit=
            monthly_limit, period=BudgetPeriod.MONTHLY,
            soft_limit_percentage=0.9, hard_limit_percentage=1.0,
            auto_reset=True))
    ...
    def invoke_chat(self, model: str, messages: list[dict[str, str]], *,
        tools: (list[dict[str, Any]] | None)=None, tool_choice: (str | None
        )=None, temperature: float=0.7, max_tokens: (int | None)=None,
        stream: bool=False, user_id: (str | None)=None, project_id: (str |
        None)=None, extra: (dict[str, Any] | None)=None) ->(dict[str, Any] |
        ...
        if not self.circuit_breaker.is_available():
            raise RuntimeError(
                f'Circuit breaker is OPEN for {self.provider}. Service temporarily unavailable.'
                )
        payload = self.payload_builder.build(model=model, messages=messages,
    ...
    def _invoke_non_streaming(self, payload: dict[str, Any], user_id: (str |
        None), project_id: (str | None)) ->dict[str, Any]:
        ...
        def _execute() ->dict[str, Any]:
            ...
            completion = transport.chat_completion(messages=payload[
                'messages'], model=payload['model'], tools=payload.get(
                'tools'), tool_choice=payload.get('tool_choice'),
                temperature=payload.get('temperature', 0.7), max_tokens=
                payload.get('max_tokens'))
            latency_ms = (time.perf_counter() - t0) * 1000.0
            envelope = self.response_normalizer.normalize(completion=
                completion, payload=payload, latency_ms=latency_ms,
                start_ts=start_time, end_ts=time.time(), retry_schedule=
                retry_schedule, attempts=1)
            if 'usage' in envelope:
                usage = envelope['usage']
                self.cost_manager.track_usage(model=payload['model'],
                    input_tokens=usage.get('prompt_tokens', 0),
                    output_tokens=usage.get('completion_tokens', 0),
                    user_id=user_id, project_id=project_id, request_id=
            ...
            return envelope
        try:
            result = self.circuit_breaker.call(lambda : self.retry_executor
                .execute(_execute))
            return result
        except Exception as e:
    ...
    def _invoke_streaming(self, payload: dict[str, Any], user_id: (str |
        None), project_id: (str | None)) ->Generator[dict[str, Any], None, None
        ]:
        """Execute streaming request."""
        transport = self._get_transport()
        try:
            stream = transport.chat_completion_stream(messages=payload[
                'messages'], model=payload['model'], tools=payload.get(
                'tools'), tool_choice=payload.get('tool_choice'),
                temperature=payload.get('temperature', 0.7), max_tokens=
                payload.get('max_tokens'))
            ...
            if accumulated_tokens > 0:
                self.cost_manager.track_usage(model=payload['model'],
                    ...
                    output_tokens=accumulated_tokens, user_id=user_id,
    ...
    def register_post_hook(self, hook: Callable[[dict[str, Any], dict[str,
        Any]], None]) ->None:
    ...
    def get_cost_metrics(self) ->dict[str, Any]:
        ...
        return {'total_cost': metrics.total_cost, 'total_requests': metrics
            .total_requests, 'total_input_tokens': metrics.
            total_input_tokens, 'total_output_tokens': metrics.
            total_output_tokens, 'cost_by_model': metrics.cost_by_model,
    ...
    def get_circuit_breaker_status(self) ->dict[str, Any]:
        ...
        return {'state': self.circuit_breaker.get_state().value,
            'total_requests': metrics.total_requests, 'successful_requests':
            metrics.successful_requests, 'failed_requests': metrics.
            failed_requests, 'rejected_requests': metrics.rejected_requests,
            'failure_rate': metrics.failure_rate, 'is_available': self.
            circuit_breaker.is_available()}

    def get_retry_metrics(self) ->dict[str, Any]:
        """Get retry metrics."""
        metrics = self.retry_executor.strategy.get_metrics()
        return {'total_attempts': metrics.total_attempts,
            'successful_retries': metrics.successful_retries,
            'failed_retries': metrics.failed_retries, 'total_delay':
            metrics.total_delay, 'average_attempts': metrics.
            average_attempts, 'retries_by_category': {k.value: v for k, v in
            metrics.retries_by_category.items()}}
...
def get_llm_client_service(provider: (str | None)=None) ->LLMClientService:
    ...
    if _GLOBAL_CLIENT is not None and (provider is None or provider ==
...
def invoke_chat(model: str, messages: list[dict[str, str]], **kwargs: Any) ->(
