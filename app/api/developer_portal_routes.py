...
def create_api_key():
    ...
    try:
        ...
        if not data or 'developer_id' not in data or 'name' not in data:
            return jsonify({
                'status': 'error',
                'message': 'developer_id and name are required'
            }), 400
...
def add_ticket_message(ticket_id: str):
    ...
    try:
        ...
        if not data or 'author_id' not in data or 'content' not in data:
            return jsonify({
                'status': 'error',
                'message': 'author_id and content are required'
            }), 400
...
def list_available_sdks():
    ...
    try:
        ...
        return jsonify({
            'status': 'error',
            'message': 'Failed to retrieve SDKs'
        }), 500
...
def get_code_examples():
    ...
    try:
        ...
        return jsonify({
            'status': 'error',
            'message': 'Failed to retrieve code examples'
        }), 500
