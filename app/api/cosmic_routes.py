...
def harmonize_node(node_id):
    ...
    if success:
        ...
        return jsonify({"ok": False, "error": "Failed to harmonize node"}), 500
...
def list_existential_nodes():
    ...
    pagination = query.order_by(ExistentialNode.created_at.desc()).paginate(
...
def create_consciousness():
    ...
    try:
        ...
        if not data or "entity_name" not in data or "entity_type" not in data:
            return jsonify({"ok": False, "error": "entity_name and entity_type are required"}), 400
...
def create_sece():
    ...
    try:
        ...
        if not data or "entity_name" not in data:
            return jsonify({"ok": False, "error": "entity_name is required"}), 400
...
def create_protocol():
    ...
    try:
        ...
        if not data or "protocol_name" not in data or "description" not in data:
            return (
                jsonify({"ok": False, "error": "protocol_name and description are required"}),
                400,
            )
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "status": protocol.status.value,
                    "activated_at": (
                        protocol.activated_at.isoformat() if protocol.activated_at else None
                    ),
                },
            }
        )
    else:
        return jsonify({"ok": False, "error": "Failed to activate protocol"}), 500
...
def list_protocols():
    ...
    pagination = query.order_by(ExistentialProtocol.created_at.desc()).paginate(
        page=page, per_page=per_page, error_out=False
    )
...
def create_council():
    ...
    try:
        ...
        if not data or "council_name" not in data or "purpose" not in data:
            return jsonify({"ok": False, "error": "council_name and purpose are required"}), 400
...
def get_council_analytics(council_id):
    ...
    if not council:
        return jsonify({"ok": False, "error": "Council not found"}), 404
...
def query_transparency():
    ...
    logs = CosmicGovernanceService.query_transparency_logs(
        event_type=event_type, min_understanding_level=min_understanding, limit=limit
    )
    ...
    log_list = [
        {
            ...
            "cosmic_reasoning": (
                log.cosmic_reasoning[:200] + "..."
                if len(log.cosmic_reasoning) > 200
                else log.cosmic_reasoning
            ),
...
def get_transparency_log(log_id):
    ...
    if not log:
        return jsonify({"ok": False, "error": "Transparency log not found"}), 404
...
def cosmic_stats():
    ...
    stats = {
        "existential_nodes": {
            ...
            "coherent": db.session.query(ExistentialNode)
            .filter(ExistentialNode.coherence_level >= 0.8)
            .count(),
        },
        ...
        "protocols": {
            ...
            "active": db.session.query(ExistentialProtocol)
            .filter(ExistentialProtocol.status == "ACTIVE")
            .count(),
        },
        "councils": {
...
def cosmic_health():
    ...
    try:
        ...
        return jsonify({"ok": False, "status": "degraded", "error": str(e)}), 503
