...
def create_subscription():
    ...
    try:
        ...
        if not subscription:
            return jsonify({
                'status': 'error',
                'message': 'Failed to create subscription. Plan not found.'
            }), 404
...
def record_usage(subscription_id: str):
    ...
    try:
        ...
        if not data or 'metric_type' not in data or 'quantity' not in data:
            return jsonify({
                'status': 'error',
                'message': 'metric_type and quantity are required'
            }), 400
        ...
        try:
            ...
            return jsonify({
                'status': 'error',
                'message': f"Invalid metric_type: {data['metric_type']}"
            }), 400
        ...
        if not success:
            return jsonify({
                'status': 'error',
                'message': 'Failed to record usage. Quota may be exceeded.'
            }), 429
...
def get_customer_subscriptions(customer_id: str):
    ...
    try:
        ...
        return jsonify({
            'status': 'error',
            'message': 'Failed to retrieve customer subscriptions'
        }), 500
