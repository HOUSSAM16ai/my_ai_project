...
def list_nodes(limit):
    ...
    nodes = db.session.query(ExistentialNode).order_by(
    ...
    for node in nodes:
        ...
        click.echo(f"ID: {node.id} | {click.style(node.status.value, fg=status_color)} | Coherence: {node.coherence_level:.2f}")
        click.echo(f"  Signature: {node.existential_signature[:64]}...")
        click.echo(f"  Dimension: {node.dimension_layer} | Interactions: {node.interaction_count}\n")
...
def show_ledger(limit, event_type):
    ...
    for entry in entries:
        click.echo(f"🔗 {entry.event_type} | {entry.cosmic_timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
...
def list_protocols(status):
    ...
    for proto in protocols:
        ...
        click.echo(f"ID: {proto.id} | {click.style(proto.status.value, fg=status_color)} | v{proto.protocol_version}")
...
def list_councils():
    ...
    for council in councils:
        ...
        click.echo(f"ID: {council.id} | {click.style('ACTIVE' if council.is_active else 'INACTIVE', fg=active_color)}")
        click.echo(f"  Name: {council.council_name}")
...
def query_logs(event_type, limit):
    ...
    for log in logs:
        ...
        click.echo(f"   Views: {log.view_count} | Understanding Level: {log.understanding_level_required}\n")
...
def show_stats():
    ...
    coherent_nodes = db.session.query(ExistentialNode).filter(
    ...
    active_seces = db.session.query(SelfEvolvingConsciousEntity).filter(
    ...
    active_protocols = db.session.query(ExistentialProtocol).filter(
    ...
    active_councils = db.session.query(CosmicGovernanceCouncil).filter(
