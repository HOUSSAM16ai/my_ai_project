...
from app.services import generation_service, system_service, history_service
...
def query_file_content(file_path, line):
    ...
    if 'line' in result:
        click.secho(f"Line {result['line']}/{result.get('total_lines', 'N/A')} in '{result.get('path', file_path)}':", fg="green")
    else:
        click.secho(f"Full content of '{result.get('path', file_path)}' ({result.get('total_lines', 'N/A')} lines):", fg="green")
...
def show_history(limit):
    ...
    click.secho(f"--- Fetching last {limit} conversations from the Akashic Records ---", fg="magenta")
    ...
    for conv in conversations:
        click.secho(f"\n[Conversation ID: {conv.id} | User: {conv.user.email} | Started: {conv.start_time.strftime('%Y-%m-%d %H:%M')}]", bg="blue", fg="white")
        ...
        for msg in conv.messages:
            ...
            click.secho(f"  > [{msg.id}] {msg.role.upper()}{rating_str}: {content_preview}", fg=role_color)
