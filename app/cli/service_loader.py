...
class ModuleImporter:
    ...
    def get_record(self, path: str) -> ImportRecord | None:
    ...
    def has_failures(self, paths: list[str]) -> bool:
        ...
        return any(rec.name in paths and not rec.ok for rec in self.imports.values())

    def get_failed_modules(self, paths: list[str] = None) -> list[ImportRecord]:
        """الحصول على قائمة الوحدات الفاشلة"""
        if paths:
            return [rec for rec in self.imports.values() if rec.name in paths and not rec.ok]
...
# ======================================================================================
_loader_instance: ServiceLoader | None = None
