...
class SLITracker:
    ...
    def record_measurement(self, sli_name: str, value: float, timestamp: (
        datetime | None)=None):
    ...
    def get_sli_value(self, sli_name: str, window_seconds: (int | None)=None
        ) ->(float | None):
        ...
        with self.lock:
            ...
            recent_measurements = [v for t, v in sli.measurements if t >=
                cutoff]
            ...
            if sli.sli_type == SLIType.AVAILABILITY:
                ...
                if len(recent_measurements) < 2:
                    return recent_measurements[0
                        ] if recent_measurements else None
                return statistics.quantiles(recent_measurements, n=100)[98]
    ...
    def get_all_slis(self) ->dict[str, dict[str, Any]]:
        ...
        with self.lock:
            return {name: {'type': sli.sli_type.value, 'description': sli.
                description, 'current_value': sli.current_value,
                'target_value': sli.target_value, 'last_updated': sli.
                last_updated.isoformat() if sli.last_updated else None,
                'measurement_count': len(sli.measurements)} for name, sli in
                self.slis.items()}
...
class SLOService:
    ...
    def _initialize_default_slos(self):
        ...
        availability_sli = SLI(name='api_availability', sli_type=SLIType.
            AVAILABILITY, description=
            'Percentage of successful API requests', measurement_window=60,
            target_value=99.9)
        self.sli_tracker.register_sli(availability_sli)
        self.slos['availability_30d'] = SLO(slo_id='slo_avail_30d', name=
            'API Availability (30d)', description=
            '99.9% of requests succeed over 30 days', sli_name=
            'api_availability', target=99.9, window_days=30)
        latency_sli = SLI(name='api_latency_p99', sli_type=SLIType.LATENCY,
            description='99th percentile API response time',
            measurement_window=60, target_value=500.0)
        self.sli_tracker.register_sli(latency_sli)
        self.slos['latency_30d'] = SLO(slo_id='slo_lat_30d', name=
            'API Latency P99 (30d)', description=
            '99% of requests complete within 500ms over 30 days', sli_name=
            'api_latency_p99', target=99.0, window_days=30)
        error_rate_sli = SLI(name='api_error_rate', sli_type=SLIType.
            ERROR_RATE, description=
            'Percentage of API requests resulting in errors',
            measurement_window=60, target_value=0.1)
        self.sli_tracker.register_sli(error_rate_sli)
        self.slos['error_rate_30d'] = SLO(slo_id='slo_err_30d', name=
            'API Error Rate (30d)', description=
            'Less than 0.1% error rate over 30 days', sli_name=
            'api_error_rate', target=99.9, window_days=30)

    def record_request(self, endpoint: str, method: str, status_code: int,
        response_time_ms: float, timestamp: (datetime | None)=None):
        """Record an API request for SLI/SLO tracking"""
        if timestamp is None:
            timestamp = datetime.now(UTC)
        is_success = 200 <= status_code < 500
        self.sli_tracker.record_measurement('api_availability', 100.0 if
            is_success else 0.0, timestamp)
        is_fast = response_time_ms <= 500.0
        self.sli_tracker.record_measurement('api_latency_p99', 100.0 if
            is_fast else 0.0, timestamp)
        is_error = status_code >= 400
        self.sli_tracker.record_measurement('api_error_rate', 0.0 if not
            is_error else 100.0, timestamp)
    ...
    def _update_slo_measurements(self):
        ...
        with self.lock:
            for slo_id, slo in self.slos.items():
                ...
                actual_value = self.sli_tracker.get_sli_value(slo.sli_name,
                    window_seconds)
                ...
                if is_compliant:
                    ...
                    error_consumed = (slo.target - actual_value
                ...
                measurement = SLOMeasurement(timestamp=datetime.now(UTC),
                    slo_id=slo_id, actual_value=actual_value, target_value=
                    slo.target, compliance=is_compliant,
    ...
    def calculate_burn_rate(self, slo_id: str) ->(ErrorBudgetBurn | None):
        ...
        with self.lock:
            ...
            measurements_1h = [m for m in self.measurements if m.slo_id ==
                slo_id and m.timestamp >= now - timedelta(hours=1)]
            measurements_6h = [m for m in self.measurements if m.slo_id ==
                slo_id and m.timestamp >= now - timedelta(hours=6)]
            measurements_24h = [m for m in self.measurements if m.slo_id ==
                slo_id and m.timestamp >= now - timedelta(hours=24)]
            measurements_7d = [m for m in self.measurements if m.slo_id ==
                slo_id and m.timestamp >= now - timedelta(days=7)]
            ...
            burn = ErrorBudgetBurn(slo_id=slo_id, timestamp=now,
                burn_rate_1h=burn_1h, burn_rate_6h=burn_6h, burn_rate_24h=
                burn_24h, burn_rate_7d=burn_7d, level=level,
    ...
    def get_slo_status(self, slo_id: str) ->(dict[str, Any] | None):
        ...
        with self.lock:
            ...
            return {'slo_id': slo.slo_id, 'name': slo.name, 'description':
                slo.description, 'target': slo.target, 'window_days': slo.
                window_days, 'error_budget_total': slo.error_budget,
    ...
    def get_dashboard(self) ->dict[str, Any]:
        ...
        with self.lock:
            return {'timestamp': datetime.now(UTC).isoformat(), 'slis':
                self.sli_tracker.get_all_slis(), 'slos': {slo_id: self.
                get_slo_status(slo_id) for slo_id in self.slos},
                'active_incidents': len([i for i in self.incidents.values() if
                i.ended_at is None]), 'overall_health': self.
    ...
    def _calculate_overall_health(self) ->str:
        ...
        if any(s == SLOStatus.BREACHED for s in statuses) or any(s ==
            SLOStatus.CRITICAL for s in statuses):
            return 'critical'
