...
class LLMSettings:
    ...
    def __init__(self):
        self.extreme_mode = os.getenv("LLM_EXTREME_COMPLEXITY_MODE", "0") == "1"
        self.ultimate_mode = os.getenv("LLM_ULTIMATE_COMPLEXITY_MODE", "0") == "1"

        default_retries = "20" if self.ultimate_mode else ("8" if self.extreme_mode else "2")
        self.max_retries = int(os.getenv("LLM_MAX_RETRIES", default_retries))

        default_backoff = "1.8" if self.ultimate_mode else ("1.5" if self.extreme_mode else "1.3")
        self.retry_backoff_base = float(os.getenv("LLM_RETRY_BACKOFF_BASE", default_backoff))
...
class LLMRequestExecutor:
        ...
        while attempts <= self.settings.max_retries:
            ...
            try:
                ...
                return self._process_success(
                    response, payload, attempts, start_time, latency_ms
                )
    ...
    def _process_success(
        self, completion: Any, payload: LLMPayload, attempts: int, start_time: float, latency_ms: float
    ) -> dict[str, Any]:
        # Extract Data
        ...
        content = ResponseSanitizer.sanitize(content, self.settings.sanitize_output)

        pt, ct, total = usage["prompt_tokens"], usage["completion_tokens"], usage["total_tokens"]
    ...
    def _handle_error(self, exc: Exception, model: str, attempts: int, backoff: float):
        kind = RetryStrategy.classify_error(exc)
        self.breaker.note_error()
        self.cost_manager.update_metrics(None, None, None, 0, None, error_kind=kind)

        if self.settings.log_attempts:
            _LOG.warning(f"LLM attempt #{attempts} failed. Kind: {kind}. Msg: {exc}")

        if attempts > self.settings.max_retries or not RetryStrategy.is_retry_allowed(kind):
            if self.settings.log_attempts:
                _LOG.error(f"LLM final failure. Kind: {kind}")
            raise exc

    def stream_generator(self, payload: LLMPayload) -> Generator[dict[str, Any], None, None]:
        envelope = self.execute(payload)
        full = envelope["content"]

        # Simulate streaming from the full response (legacy behavior maintained)
        def _maybe_stream_simulated(full_text: str, chunk_size: int = 100):
            if not full_text:
                return
        ...
        for chunk in _maybe_stream_simulated(full):
            yield {"delta": ResponseSanitizer.sanitize(chunk, self.settings.sanitize_output)}
