...
class PacingStrategy(Protocol):
    ...
    def delay_ms(self, metrics: StreamingMetrics, config: StreamingConfig) -> float:  # pragma: no cover - بروتوكول
...
class AdaptivePacingStrategy:
    ...
    def delay_ms(self, metrics: StreamingMetrics, config: StreamingConfig) -> float:
        ...
        latest_latency = metrics.chunk_times[-1] if metrics.chunk_times else config.min_chunk_delay_ms
