...
class TraceContextPropagator:
    ...
    def inject(span_context: SpanContext, headers: dict[str, str]) -> None:
        ...
        if span_context.baggage:
            tracestate = ','.join(f'{k}={v}' for k, v in span_context.
                baggage.items())
    ...
    def extract(headers: dict[str, str]) ->(SpanContext | None):
        ...
        try:
            ...
            baggage = {}
            tracestate = headers.get(TraceContextPropagator.
                TRACESTATE_HEADER, '')
            if tracestate:
            ...
            return SpanContext(trace_id=trace_id, span_id=span_id,
                parent_span_id=span_id, baggage=baggage)
...
class DistributedTracer:
    ...
    def __init__(self, service_name: str='cogniforge', sampling_strategy:
        SamplingStrategy=SamplingStrategy.ALWAYS, sampling_rate: float=1.0):
        ...
        self.metrics = {'total_traces': 0, 'total_spans': 0,
            'traces_with_errors': 0, 'avg_trace_duration_ms': 0}

    def start_trace(self, operation_name: str, kind: SpanKind=SpanKind.
        SERVER, parent_context: (SpanContext | None)=None) ->SpanContext:
        """
        ...
        span = Span(span_id=span_id, trace_id=trace_id, operation_name=
            operation_name, service_name=self.service_name, kind=kind,
            start_time=datetime.now(UTC), parent_span_id=parent_span_id,
            baggage=parent_context.baggage if parent_context else {})
        with self.lock:
            self.active_spans[span_id] = span
        return SpanContext(trace_id=trace_id, span_id=span_id,
            parent_span_id=parent_span_id, baggage=span.baggage.copy())

    def end_span(self, span_context: SpanContext, status_code: str='OK',
        status_message: (str | None)=None):
        """End a span"""
    ...
    def add_span_log(self, span_context: SpanContext, message: str, fields:
    ...
    def _try_aggregate_trace(self, trace_id: str):
        ...
        with self.lock:
            ...
            if not spans:
                return
            active_for_trace = [s for s in self.active_spans.values() if s.
                trace_id == trace_id]
            if active_for_trace:
                return
            root_span = min(spans, key=lambda s: s.start_time)
            last_span = max(spans, key=lambda s: s.end_time or s.start_time)
            trace = Trace(trace_id=trace_id, root_span_id=root_span.span_id,
                service_name=root_span.service_name, start_time=root_span.
                ...
                for span in trace.spans:
                    if span.parent_span_id:
                        for parent_span in trace.spans:
                            if (parent_span.span_id == span.parent_span_id and
                                parent_span.service_name != span.service_name):
                                dependencies[parent_span.service_name].add(span
                                    .service_name)
    ...
    def get_metrics(self) ->dict[str, Any]:
        ...
        with self.lock:
            ...
            completed_with_duration = [t for t in self.traces.values() if t
                .duration_ms is not None]
            avg_duration = sum(t.duration_ms for t in completed_with_duration
                ) / len(completed_with_duration
                ) if completed_with_duration else 0
            return {'active_spans': active_spans_count, 'pending_traces':
                pending_traces, 'completed_traces': completed_traces,
                'total_traces': self.metrics['total_traces'], 'total_spans':
                self.metrics['total_spans'], 'traces_with_errors': self.
                metrics['traces_with_errors'], 'avg_trace_duration_ms':
                avg_duration, 'error_rate': self.metrics[
                'traces_with_errors'] / self.metrics['total_traces'] * 100 if
                self.metrics['total_traces'] > 0 else 0}
