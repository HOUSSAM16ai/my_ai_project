...
def get_recent_conversations(limit: int = 5):
    ...
    try:
        ...
        current_app.logger.error(f"Failed to fetch recent conversations for user {current_user.id}: {e}", exc_info=True)
        return []
...
def rate_message_in_db(message_id: int, rating: str):
    ...
    try:
        ...
        if message_to_rate.conversation.user_id != current_user.id:
            current_app.logger.warning(f"SECURITY ALERT: User {current_user.id} tried to rate message {message_id} belonging to another user.")
            return {"status": "error", "message": "Permission denied. You can only rate your own conversations."}
        
        message_to_rate.rating = rating
        db.session.commit()
        
        current_app.logger.info(f"User {current_user.id} rated message {message_id} as '{rating}'.")
        return {"status": "success", "message": f"Message {message_id} has been rated as '{rating}'."}
        
    except sqlalchemy_exc.SQLAlchemyError as e:
        db.session.rollback()
        current_app.logger.error(f"Database error while rating message {message_id}: {e}", exc_info=True)
        return {"status": "error", "message": "A database error occurred."}
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Unexpected error while rating message {message_id}: {e}", exc_info=True)
