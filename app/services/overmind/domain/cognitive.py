...
from app.services.overmind.domain.primitives import CognitiveState, EventLogger
...
class SuperBrain:
    ...
    Orchestrates the cognitive phases using Strategy Pattern.
    """
    ...
    def __init__(
        ...
        # Initialize Runner
        self.runner = CognitivePhaseRunner(memory_agent=memory_agent)

        # Initialize Phase Strategies
        self.planning_phase = PlanningPhase(self.runner, strategist, auditor)
        self.design_phase = DesignPhase(self.runner, architect)
        self.execution_phase = ExecutionPhase(self.runner, operator)
        self.reflection_phase = ReflectionPhase(self.runner, auditor)
    ...
    async def _execute_cognitive_cycle(
        ...
        await self.design_phase.execute(state, collab_context, safe_log, session)

        # المرحلة 3: التنفيذ | Execution phase
        await self.execution_phase.execute(state, collab_context, safe_log, session)

        # المرحلة 4: المراجعة | Review phase
        await self.reflection_phase.execute(state, collab_context, safe_log, session)
