...
class PredictiveTaskAnalyzer:
    ...
    def _calculate_similarity(self, task1: Task, task2: Task) -> float:
        ...
        if task1.estimated_hours > 0 and task2.estimated_hours > 0:
            ratio = min(task1.estimated_hours, task2.estimated_hours) / max(task1.estimated_hours, task2.estimated_hours)
            score += ratio * 0.3
...
class SmartScheduler:
    ...
    def _find_best_assignee(
        ...
        task: Task,
        team: List[TeamMember],
        current_assignments: Dict[str, List[Task]]
    ) -> Optional[TeamMember]:
        """إيجاد أفضل عضو فريق للمهمة"""
        scored_members = []
        
        for member in team:
            score = self._calculate_assignment_score(task, member, current_assignments[member.member_id])
...
class ProjectOrchestrator:
    ...
    def generate_smart_insights(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'timeline': {
                ...
                'predicted_completion_date': (datetime.now() + timedelta(days=days_to_completion)).isoformat()
            },
            'progress': {
            ...
            'team_utilization': {
                'average_capacity': sum(m.availability() for m in self.team) / len(self.team) if self.team else 0,
                'overloaded_members': sum(1 for m in self.team if m.current_workload > m.capacity_hours_per_day * 5 * 1.2)
            },
    ...
    def _generate_recommendations(
        ...
        if completion_rate < 0.3:
            recommendations.append("🚀 Project is in early stages. Focus on establishing clear goals and priorities.")
        ...
        if bottlenecks:
            recommendations.append(f"🔧 {len(bottlenecks)} bottlenecks detected. Review and resolve blockers.")
