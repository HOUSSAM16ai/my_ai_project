...
class RefactorTool:
    ...
    def _write_atomic(self, path: Path, text: str, preserve_meta_from: Optional[Path] = None) -> None:
    ...
    def _build_diff(self, old: str, new: str, file_path: str) -> str:
        ...
        diff_lines = difflib.unified_diff(a, b, fromfile=f"{file_path} (original)", tofile=f"{file_path} (refactored)")
    ...
    def _run_formatters(self, file_path: Path) -> None:
        for cmd in self.formatter_cmds:
            ...
            try:
                subprocess.run(cmd_expanded, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def apply_code_refactoring(
        ...
        if len(original) > self.max_chars:
            return RefactorResult(file_path, False, False, None, "", "File is too large to process.")
        ...
        if not isinstance(refactored, str) or not refactored.strip():
            return RefactorResult(file_path, False, False, None, "", "Refactoring result from AI was empty.")

        diff_text = self._build_diff(original, refactored, file_path)
        if not diff_text:
            return RefactorResult(file_path, False, False, None, "", "No changes were generated by the AI.")

        if dry_run:
            return RefactorResult(file_path, True, False, None, diff_text, "Dry-run: changes preview only.")
        ...
        try:
            ...
            return RefactorResult(file_path, True, False, backup_path, diff_text, f"Write error: {e}")
        
        self._run_formatters(p)
        
        return RefactorResult(file_path, True, True, backup_path, diff_text, "Refactoring applied successfully.")