...
class OnCallIncidentService:
    ...
    def get_incident_metrics(self) -> dict[str, Any]:
        ...
        with self.lock:
            ...
            resolved_incidents = [i for i in self.incidents.values() if i.resolved_at is not None]

            mttr_minutes = 0
            if resolved_incidents:
                total_resolution_time = sum(
                    (i.resolved_at - i.detected_at).total_seconds() / 60 for i in resolved_incidents
                )
