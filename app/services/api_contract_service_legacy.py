...
# New code should use: from app.services.api_contract import get_api_contract_service
...
APIVersion = type(_service.get_version_info("v1") or object())
...
class APIContractService:
    ...
    def validate_request(
        ...
        try:
            ...
            self._log_contract_violation(
                ...
                severity="high",
                details={"validation_errors": errors, "data_type": "request"},
                actual_data=data,
            )
    ...
    def _normalize_endpoint(self, endpoint: str) -> str:
        ...
        for i, part in enumerate(parts):
            ...
            # Check if this is a table name after /record/ path (only when it's the last part)
                ...
                i > 0
                and parts[i - 1] == "record"
                and "/api/database/record/" in endpoint
                and i == len(parts) - 1
            ):
                normalized_parts.append("{table_name}")
            else:
    ...
    def generate_openapi_spec(self) -> dict[str, Any]:
        ...
        return {
            ...
            "servers": [
                {"url": "/admin/api", "description": "Admin API v2 (Current)"},
                {"url": "/admin/api/v1", "description": "Admin API v1 (Deprecated)"},
            ],
            "components": {
                "schemas": {
                    **BASE_SCHEMAS,
    ...
    def _generate_paths(self) -> dict[str, Any]:
        ...
        for endpoint, methods in ENDPOINT_SCHEMAS.items():
            ...
            for method, specs in methods.items():
                operation = {"summary": f"{method} {endpoint}", "responses": {}}

                if "request" in specs:
                    operation["requestBody"] = {
                        "required": True,
                        "content": {"application/json": {"schema": specs["request"]}},
                    }
