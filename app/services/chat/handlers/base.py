...
class AsyncToolsProtocol(Protocol):
    ...
    async def code_search_lexical(self, query: str, limit: int=10,
        context_radius: int=3) ->dict:
        ...
...
class ChatContext:
    ...
    def __init__(self, async_tools: (AsyncToolsProtocol | None)=None,
        async_overmind: (AsyncOvermindProtocol | None)=None, rate_limiter:
        (RateLimiterProtocol | None)=None, circuit_breaker: (
        CircuitBreakerProtocol | None)=None):
        self.async_tools = async_tools
        self.async_overmind = async_overmind
        self.rate_limiter = rate_limiter
        self.circuit_breaker = circuit_breaker

    async def check_rate_limit(self, user_id: int, tool_name: str) ->tuple[
        bool, str]:
