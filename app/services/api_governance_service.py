...
class OWASPComplianceChecker:
    ...
    def check_resource_consumption(
        ...
        if computation_cost > MAX_COMPUTATION_COST:
            return False, f"Computation cost {computation_cost} exceeds limit {MAX_COMPUTATION_COST}"
            ...
            return {
                'api_versions': {
                    v: {
                        ...
                        'deprecation_date': ver.deprecation_date.isoformat() if ver.deprecation_date else None,
                        'sunset_date': ver.sunset_date.isoformat() if ver.sunset_date else None,
                        'supported_until': ver.supported_until.isoformat() if ver.supported_until else None
                    }
                    for v, ver in self.versions.items()
                },
                'active_deprecations': len([p for p in self.deprecation_policies.values() if datetime.now(timezone.utc) < p.sunset_date]),
        ...
        if hasattr(response, 'headers'):
            if version_info.status == APIVersionStatus.DEPRECATED:
                ...
                if version_info.migration_guide_url:
                    response.headers['Link'] = f'<{version_info.migration_guide_url}>; rel="deprecation"'
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            if current_user.is_authenticated:
                ...
                client_type = 'premium' if hasattr(current_user, 'is_premium') and current_user.is_premium else 'authenticated'
            else:
                client_type = 'anonymous'
        except:
            client_type = 'anonymous'
