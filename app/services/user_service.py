...
class UserService:
    ...
    def __init__(
        ...
        _logger: logging.Logger | None = None,  # Accept but don't require logger
    ):
        """
    ...
    async def _get_session(self):
        ...
        return None  # Signal to use context manager
    ...
    async def _create_new_user_with_session(
        self, session: AsyncSession, full_name: str, email: str, password: str, is_admin: bool
    ) -> dict[str, str]:
        ...
        try:
            ...
            logger.info(f"User '{full_name}' created with ID {new_user.id}{admin_status}.")
            return {
                "status": "success",
...
async def create_new_user_async(
    ...
    return await get_user_service().create_new_user(full_name, email, password, is_admin)


async def ensure_admin_user_exists_async() -> dict[str, str]:
    """Async function to ensure admin user exists."""
    return await get_user_service().ensure_admin_user_exists()
