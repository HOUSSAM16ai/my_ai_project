# app/services/analytics/domain/models.py
"""
...
class EngagementMetrics:
    ...
    def stickiness(self) -> float:
        ...
        return self.dau / self.mau if self.mau > 0 else 0.0
...
class RetentionMetrics:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_user_lifetime": self.avg_user_lifetime,
        }
...
class ABTestResults:
    ...
    def record_variant_view(self, variant: ABTestVariant) -> None:
        """Record a variant view"""
...
class CohortAnalysis:
    ...
    retention_data: dict[int, float] = field(default_factory=dict)  # day -> retention_rate
    
    def add_user(self, user_id: int) -> None:
        """Add user to cohort"""
        self.users.add(user_id)
    
    def record_retention(self, days_since_creation: int, active_users: int) -> None:
        """Record retention rate for specific day"""
        total_users = len(self.users)
        retention_rate = (active_users / total_users) if total_users > 0 else 0.0
        self.retention_data[days_since_creation] = retention_rate
...
class RevenueMetrics:
    ...
    def record_transaction(
        ...
        self.unique_paying_users.add(user_id)
        
        if segment:
            if segment not in self.revenue_by_segment:
                self.revenue_by_segment[segment] = 0.0
            self.revenue_by_segment[segment] += amount
    
    @property
    def average_transaction_value(self) -> float:
        """Calculate average transaction value"""
        return (self.total_revenue / self.total_transactions) if self.total_transactions > 0 else 0.0
    
    @property
    def arpu(self) -> float:
        """Calculate Average Revenue Per User"""
        total_users = len(self.unique_paying_users)
        return (self.total_revenue / total_users) if total_users > 0 else 0.0
