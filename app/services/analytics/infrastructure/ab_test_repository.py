# app/services/analytics/infrastructure/ab_test_repository.py
"""
...
class InMemoryABTestRepository(ABTestManagerPort):
    ...
    def create_test(
        ...
        with self._lock:
            ...
            self._conversions[test_id] = {variant: set() for variant in variants}
    ...
    def get_test_results(
        ...
        with self._lock:
            ...
            for variant in variants:
                assigned_count = sum(
                    1
                    for v in self._user_assignments[test_id].values()
                    if v == variant
                )
                conversion_count = len(self._conversions[test_id][variant])
                conversion_rate = (
                    conversion_count / assigned_count
                    if assigned_count > 0
                    else 0.0
                )
                
                variant_stats[variant.value] = {
    ...
    def stop_test(self, test_id: str) -> None:
        ...
        with self._lock:
            ...
            self._tests[test_id]["stopped_at"] = datetime.utcnow()
