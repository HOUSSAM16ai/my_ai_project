...
class InMemoryBroker(MessageBroker):
    ...
    def _execute_handler(self, subscription: EventSubscription, message: Event):
        ...
        current_app.logger.error(
            f"Event {message.event_id} failed after {max_retries} retries"
        )
...
class EventDrivenService:
    ...
    def publish(
        ...
        event_id = hashlib.sha256(
            f"{event_type}{datetime.now(timezone.utc)}".encode()
        ).hexdigest()[:16]
        ...
        if success:
            current_app.logger.info(
                f"Event published: {event_type} ({event_id})"
            )
        else:
            current_app.logger.error(
                f"Failed to publish event: {event_type} ({event_id})"
            )
    ...
    def subscribe(
        ...
        current_app.logger.info(
            f"Subscribed to event type: {event_type} ({subscription_id})"
        )
