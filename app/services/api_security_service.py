...
def rate_limit(f: Callable) -> Callable:
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            client_id = str(current_user.id) if current_user.is_authenticated else request.remote_addr
        except:
        ...
        if hasattr(response, 'headers'):
            response.headers['X-RateLimit-Limit'] = str(limit_info.get('limit', RATE_LIMIT_REQUESTS))
