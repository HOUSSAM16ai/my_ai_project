...
import os, re, math, time, logging, json, glob
...
def _env_flag(name: str, default: bool=False) -> bool:
    return os.getenv(name, str(int(1 if default else 0))).strip().lower() in ("1","true","yes","on")
...
EXTRA_READ_GLOBS = [g.strip() for g in os.getenv("PLANNER_EXTRA_READ_GLOBS","").split(",") if g.strip()]
...
SCAN_EXTS = {e.strip().lower() for e in os.getenv("PLANNER_SCAN_EXTS",".py").split(",") if e.strip()}
SCAN_MAX_FILES = _env_int("PLANNER_SCAN_MAX_FILES", 140)

OPTIONAL_GROUPS = [g.strip() for g in os.getenv(
    "PLANNER_OPTIONAL_TASK_GROUPS",
    "semantic,global_summary,deep_arch_report"
).split(",") if g.strip()]
...
def _ensure_ext(fn: str)->str: return fn if "." in fn else fn+DEFAULT_EXT

def extract_filenames(obj:str)->List[str]:
    norm=" ".join(obj.split()); out=[]
    ...
    if not out:
        ...
        for g in guess:
            ...
            if g.lower() not in [x.lower() for x in out]:
                out.append(g)
            if len(out)>=MAX_FILES: break
    if not out: out=["output"+DEFAULT_EXT]
...
def compute_chunk_plan(requested:Optional[int])->Tuple[int,int]:
    ...
    if requested <= FAST_SINGLE_THRESH: return 1, requested
...
def code_hint(ftype:str, lang:str)->str:
    ...
    if ftype=="code":
        return "أضف أمثلة كود وتعليقات عميقة.\n" if lang=='ar' else "Add deep code samples & commentary.\n"
    if ftype=="data":
        return "أضف سجلات نموذجية وشرح الحقول.\n" if lang=='ar' else "Provide sample records & field descriptions.\n"
    return ""
...
# Prompt Builders
# --------------------------------------------------------------------------------------
def build_role_prompt(files:List[str], objective:str, lang:str, struct_ref:Optional[str]=None)->str:
    listing=", ".join(files)
    struct_block=f"\nSTRUCT_CONTEXT:\n{struct_ref}\n" if struct_ref else ""
    if lang=="ar":
...
def build_section_prompt(objective:str, draft_sections:List[str], lang:str, struct_ref:Optional[str]=None)->str:
...
def build_chunk_prompt(objective:str,fname:str,role_id:Optional[str],section_id:Optional[str],
                       ...
                       struct_placeholder:Optional[str]=None, inline_struct:str="")->str:
    ...
    guide=("- Maintain flow; no early finalization.\n- Add lists/examples/risks/refactor ideas.\n"
           "- Avoid full intro repetition.\n")
    return header+guide+code_hint(ftype,lang)

def build_final_wrap_prompt(objective:str,fname:str,role_id:Optional[str],lang:str,
                            struct_placeholder:Optional[str]=None, inline_struct:str="")->str:
    role_ref=f"{{{{{role_id}.answer}}}}" if role_id else "(no-role)"
    if struct_placeholder:
        struct_ref=f"\nSTRUCT_CONTEXT:\n{{{{{struct_placeholder}.answer}}}}"
...
def _collect_extra_files()->List[str]:
    ...
    for pattern in EXTRA_READ_GLOBS:
        for p in glob.glob(pattern, recursive=True):
            if len(collected)>=SCAN_MAX_FILES: break
            if os.path.isfile(p): collected.add(p)
        if len(collected)>=SCAN_MAX_FILES: break
    # Directories
        ...
        if SCAN_RECURSIVE:
            ...
            for f in os.listdir(base):
                ...
                if os.path.isfile(fp):
                    ext=os.path.splitext(f)[1].lower()
                    if ext in SCAN_EXTS: collected.add(fp)
        if len(collected)>=SCAN_MAX_FILES: break
...
class UltraHyperPlanner(BasePlanner):
    ...
    capabilities={"semantic","chunked","multi-file","arabic","adaptive","struct_index","architecture","telemetry","global_scan"}
    ...
    def generate_plan(self, objective:str, context:Optional[PlanningContext]=None,
        ...
        est_per_file_stream = total_chunks*2 + (2 if total_chunks>1 else 1) # think+append per chunk + wrap pair
        ...
        if projected > GLOBAL_TASK_CAP and total_chunks>1:
            ...
            if new_total < total_chunks:
                ...
                per_chunk = max(80, math.ceil((req_lines or CHUNK_SIZE_HINT*2)/max(1,total_chunks)))
            ...
            try:
                if SEMANTIC_REUSE_INDEX and deep_summary_text:
                    ...
                    if _DEEP_INDEX_ENABLED and _HAS_INDEXER:
                        ...
                        sem_source = summarize_for_prompt(index_for_sem, max_len=min(STRUCT_SEMANTIC_MAX_BYTES, DEEP_INDEX_SUMMARY_MAX))
                    else:
                ...
                struct_semantic_task=_tid(idx); idx+=1
                tasks.append(PlannedTask(
        ...
        if GLOBAL_CODE_SUMMARY_EN and extra_read_ids:
            try:
                ...
                global_code_summary_task=_tid(idx); idx+=1
                tasks.append(PlannedTask(
        ...
        if ROLE_DERIVATION and len(files)>1:
            ...
            tasks.append(PlannedTask(
                ...
                tool_name=TOOL_THINK,
                tool_args={"prompt": build_role_prompt(files, objective, lang, struct_ref=_truncate(ref or "",1100))},
        ...
        if inferred_sections:
            ...
            tasks.append(PlannedTask(
                ...
                tool_name=TOOL_THINK,
                tool_args={"prompt": build_section_prompt(objective,inferred_sections,lang,struct_ref=_truncate(ref or "",900))},
                dependencies=[role_task_id] if role_task_id else (index_deps or analysis_dependency_ids)
            ))

        # ---------- File generation ----------
        final_writes=[]
        idx=self._add_file_generation_blocks(
            tasks=tasks, idx=idx, files=files, objective=objective, lang=lang,
            role_task_id=role_task_id, section_task_id=section_task_id,
            analysis_deps=(index_deps or analysis_dependency_ids),
            total_chunks=total_chunks, per_chunk=per_chunk, use_stream=use_stream,
            final_writes=final_writes, struct_placeholder=struct_placeholder_ref,
        ...
        _LOG.info("[Planner v7.3.0] tasks=%d pruned=%d streaming=%s ctx=%s semantic=%s global=%s ms=%.1f",
                  len(tasks), len(tasks_pruned), use_stream, context_source,
                  bool(struct_semantic_task), bool(global_code_summary_task), elapsed)
        return plan
    ...
    def _attempt_deep_index(self, tasks:List[PlannedTask], idx:int,
        ...
        try:
            ...
            struct_meta.update({
                ...
                "hotspot_count": len(index_data.get("complexity_hotspots_top50", [])) if index_data.get("complexity_hotspots_top50") else None,
                "duplicate_groups": len(index_data.get("duplicate_function_bodies", {})) if index_data.get("duplicate_function_bodies") else None,
            ...
            # MD summary
            if DEEP_INDEX_MD_EN:
                header="## Structural AST Index Summary\n" if lang!="ar" else "## ملخص الفهرسة البنائية (AST)\n"
                md_content=header+"\n```\n"+summary_text+"\n```\n"
                md_task=_tid(idx); idx+=1
                tasks.append(PlannedTask(
                    task_id=md_task,
    ...
    def _truncate_json(self, data:Dict[str,Any], max_bytes:int)->str:
        ...
        if len(raw.encode("utf-8"))<=max_bytes: return raw
        slim=dict(data)
        if "modules" in slim:
            slim["modules"]=[
                {"path": m.get("path"), "fn_count": len(m.get("functions",[])),
                 "class_count": len(m.get("classes",[]))}
                for m in slim["modules"][:250]
            ]
        raw2=json.dumps(slim, ensure_ascii=False, separators=(",",":"))
        if len(raw2.encode("utf-8"))<=max_bytes: return raw2
            ...
            if use_stream and self._append_allowed() and total_chunks>1:
                ...
                tasks.append(PlannedTask(
                    ...
                    tool_args={"prompt": build_final_wrap_prompt(objective,fname,role_task_id,lang,
                                                                 struct_placeholder, inline_struct)},
                    dependencies=[prev] if prev else base_deps
                ))
                wrap_append=_tid(idx); idx+=1
                tasks.append(PlannedTask(
                    task_id=wrap_append,
    ...
    def _maybe_add_artifact_index(self, tasks:List[PlannedTask], idx:int, lang:str,
                                  deps:List[str], files:List[str])->int:
    ...
    def _maybe_add_deep_arch_report(self, tasks:List[PlannedTask], idx:int, lang:str,
                                    deps:List[str], struct_meta:Dict[str,Any])->Optional[Dict[str,Any]]:
        if not struct_meta.get("attached"): return None
        prompt_ar=("حلل بيانات الفهرس (JSON + ملخص) وقدم تقرير معمارية متقدم "
                   "(طبقات، خدمات، تبعيات، نقاط ساخنة، تكرار، أولويات refactor، مخاطر، فرص تحسين). "
                   "Markdown منظم مختصر.")
        prompt_en=("Analyze structural index (JSON + summary) → advanced architecture report "
...
Provide deep, organized analysis with superhuman intelligence in one comprehensive file."""
    ...
    def _prune_if_needed(self, tasks:List[PlannedTask], idx:int, final_writes:List[str])->Tuple[int,List[str]]:
        ...
        for group in OPTIONAL_GROUPS:
            if len(tasks) <= GLOBAL_TASK_CAP: break
    ...
    def _resolve_target_files(self, objective:str)->List[str]:
        raw=extract_filenames(objective); normalized=[]
        for f in raw:
    ...
    def _append_allowed(self)->bool: return self._can_stream()
    ...
    def _initial_banner(self, fname:str, objective:str, lang:str)->str:
        ext=fname.lower(); trunc=objective[:220]
        if ext.endswith((".md",".txt",".log",".rst",".adoc",".html")):
            return (f"# تهيئة: {fname}\n\n> الهدف: {trunc}...\n\n") if lang=='ar' else (f"# Init: {fname}\n\n> Objective: {trunc}...\n\n")
    ...
    def _validate(self, plan:MissionPlanSchema, files:List[str]):
        ...
        if STRICT_WRITE_ENF:
            for f in files:
                if not any((tt.tool_name in (TOOL_WRITE,TOOL_APPEND)) and (tt.tool_args or {}).get("path","").lower()==f.lower()
...
__all__=[
    "UltraHyperPlanner","LLMGroundedPlanner",
    "MissionPlanSchema","PlannedTask","PlanningContext",
...
if __name__=="__main__":
    ...
    demo=("Analyze repository architecture, container layout and create file named "
