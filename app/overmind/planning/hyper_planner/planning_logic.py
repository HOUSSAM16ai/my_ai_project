...
def determine_streaming_strategy(total_chunks: int, can_stream: bool) -> bool:
    ...
    """
    return (
        can_stream and config.STREAM_ENABLE and total_chunks >= config.STREAM_MIN_CHUNKS
    )
...
def prune_tasks_if_needed(
    tasks: list[PlannedTask], idx: int, final_writes: list[str]
) -> tuple[int, list[str]]:
    """
...
def read_from_file(file_path: str) -> Any:
    ...
    Returns:
        File content (parsed dict for YAML/JSON, string for text, None on error)
    """
        ...
        for d in t.dependencies:
            if d not in ids:
                raise PlanValidationError(
                    f"dangling_dependency:{t.task_id}->{d}", planner_name, objective
                )
    
    # Check for missing file writes (if strict enforcement is enabled)
    if config.STRICT_WRITE_ENF:
        for f in files:
            if not any(
                ...
                raise PlanValidationError(f"missing_file_write:{f}", planner_name, objective)
