...
class SelectionProfiler:
    ...
    def record_selection(self, objective_len: int, required_caps: list[str],
        best_planner: str, score: float, candidates_count: int,
        deep_context: bool, hotspots_count: int, breakdown: dict[str, Any],
        duration_s: float, boost_config: dict[str, Any]):
        """Record a planner selection event."""
        if not self._enabled:
            return
        sample = {'objective_len': objective_len, 'required_caps': sorted(
            required_caps), 'best': best_planner, 'score': score,
            'candidates_considered': candidates_count, 'deep_index':
            deep_context, 'hotspots': hotspots_count, 'breakdown':
            breakdown, 'boost_config': boost_config, 'duration_s':
            duration_s, 'ts': time.time()}
...
class InstantiationProfiler:
    ...
    def record_instantiation(self, planner_name: str, duration_s: float,
        success: bool=True):
        """Record a planner instantiation event."""
        if not self._enabled:
            return
        sample = {'name': planner_name, 'duration_s': duration_s, 'success':
            success, 'ts': time.time()}
        self._buffer.push(sample)
...
class TelemetryManager:
    ...
    def __init__(self, max_profiles: int=1000, enable_selection: bool=True,
...
__all__ = ['InstantiationProfiler', 'RingBuffer', 'SelectionProfiler',
    'TelemetryManager']
