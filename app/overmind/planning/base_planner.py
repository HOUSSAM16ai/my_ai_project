...
# Version: 5.0.0  •  Codename: "ATOMIC FRAGMENTATION"
# ======================================================================================
# PURPOSE
#   Canonical abstract planner base class.
#   REFACTORED to delegate responsibilities to specialized sub-systems:
#     - reliability.py: Tracking success/failure stats.
#     - execution.py: Handling threading, async, and timeouts.
#     - governance.py: Configuration and Allow/Block lists.
#
#   This class now acts as the COORDINATOR (Orchestrator) rather than the MONOLITH.
# ======================================================================================
...
class BasePlanner:
    ...
    @classmethod
    def _run_self_test(cls, planner_cls: type[BasePlanner], key: str, state: ReliabilityState):
        """Delegates to extracted self-test runner module."""
        # Note: _run_self_test_impl expects the state object to have specific fields,
        # which our new ReliabilityState preserves.
    ...
    def _update_reliability(
        ...
        with cls._lock:
            ...
            if not state:
                return
            # Delegate to ReliabilityState logic
        ...
        *,
        ...
        try:
            if enforce_timeout and timeout:
                ...
                plan = run_with_timeout_sync(
                    self.generate_plan, (objective, context), self.name, objective, timeout
                )
            else:
    ...
    async def a_instrumented_generate(
        ...
        *,
        ...
        try:
            if enforce_timeout and timeout:
                # Delegate to Execution Engine
                plan = await run_with_timeout_async(
                    self.a_generate_plan(objective, context), self.name, objective, timeout
                )
            else:
