...
class AdaptiveRateLimiter:
    ...
    def check_rate_limit(
        ...
        minute_allowed = self._check_window(
            minute_key, dynamic_limit.requests_per_minute, 60, now
        )
        
        # Hour window
        hour_key = f"{identifier}:hour"
        hour_allowed = self._check_window(
            hour_key, dynamic_limit.requests_per_hour, 3600, now
        )
        
        # Day window
        day_key = f"{identifier}:day"
        day_allowed = self._check_window(
            day_key, dynamic_limit.requests_per_day, 86400, now
        )
        ...
        if not is_allowed and profile.is_legitimate and profile.behavior_score > 0.8:
            burst_allowed = self._check_burst_allowance(
                identifier, dynamic_limit.burst_allowance
            )
            if burst_allowed:
                is_allowed = True
    ...
    def _get_or_create_profile(
        ...
        if identifier not in self.user_profiles:
            self.user_profiles[identifier] = UserBehaviorProfile(
                user_id=identifier if identifier != ip_address else None,
    ...
    def _update_behavior_profile(
        ...
        night_requests = sum(
            1 for ts in profile.request_pattern
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "legitimate_users": sum(
                1 for p in self.user_profiles.values() if p.is_legitimate
            ),
            "system_load": self.system_load,
            "learning_enabled": self.enable_learning,
        }
    
    def get_user_profile(self, identifier: str) -> Optional[UserBehaviorProfile]:
