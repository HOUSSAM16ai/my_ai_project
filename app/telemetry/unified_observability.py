...
class UnifiedObservabilityService:
    ...
    def set_baggage(self, key: str, value: str, request: Request | None = None):
        if request and hasattr(request.state, "trace_context"):
            request.state.trace_context.baggage[key] = value
            if hasattr(request.state, "current_span_id"):
                span_id = request.state.current_span_id
                with self.lock:
                    if span_id in self.active_spans:
