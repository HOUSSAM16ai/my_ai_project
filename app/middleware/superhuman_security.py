...
from app.telemetry.events import EventTracker, EventType
...
class SuperhumanSecurityMiddleware:
    ...
    def __init__(
        ...
        self.zero_trust = ZeroTrustAuthenticator(
            secret_key or "change-me-in-production"
        ) if enable_zero_trust else None
            ...
            if not is_safe:
                ...
                if self.tracer and hasattr(g, 'span_id'):
                    self.tracer.end_span(
                        g.span_id,
                        status="error",
                        status_message="WAF blocked request"
                    )
