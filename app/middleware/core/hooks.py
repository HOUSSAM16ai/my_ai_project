...
def on_before_execution(callback: Callable):
    """
    Decorator to register a before_execution hook

    Usage:
        @on_before_execution
        def my_hook(ctx: RequestContext):
...
def on_after_success(callback: Callable):
    """
    Decorator to register an after_success hook

    Usage:
        @on_after_success
        def my_hook(ctx: RequestContext, result: MiddlewareResult):
            print("Success!")
    """
...
def on_after_failure(callback: Callable):
    """
...
def on_after_execution(callback: Callable):
    """
