...
Refactored to remove legacy framework fallback complexity.
"""
...
class ResponseFactory:
    ...
    def make_json_response(data: dict[str, Any], status_code: int=200,
        ...
        """
        headers = headers or {}
        return JSONResponse(content=data, status_code=status_code, headers=
            headers)

    @staticmethod
    def make_error_response(message: str, status_code: int=500, error_code:
        (str | None)=None, details: (dict[str, Any] | None)=None
        ) ->JSONResponse:
        """
        Create an error response
        """
        data: dict[str, Any] = {'error': True, 'message': message,
