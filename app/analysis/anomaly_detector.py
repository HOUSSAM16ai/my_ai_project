...
class AnomalyDetector:
    ...
    def check_value(
        ...
        is_anomaly_zscore, score_zscore = self._detect_zscore_anomaly(
            metric_name, value
        )
        
        # Method 2: IQR (Interquartile Range) detection
        is_anomaly_iqr, score_iqr = self._detect_iqr_anomaly(
            metric_name, value
        )
        
        # Method 3: Moving Average detection
        is_anomaly_ma, score_ma = self._detect_moving_average_anomaly(
            metric_name, value
        )
        ...
        if self.enable_ml:
            is_anomaly_ml, score_ml = self._detect_ml_anomaly(
                metric_name, value
            )
        ...
        if is_anomaly:
            ...
            # Get expected range
            expected_range = self.thresholds.get(
                metric_name,
                (0.0, float('inf'))
            )
    ...
    def _detect_ml_anomaly(
        ...
        similar_count = sum(
            1 for h in history
            if abs(h - value) < (statistics.stdev(history) * 0.5)
        )
    ...
    def detect_collective_anomaly(
        ...
        for data in recent_data:
            is_anom, _ = self.check_value(
                metric_name, data["value"], data.get("context")
            )
            if is_anom:
                anomalous_count += 1
