...
class RootCauseAnalyzer:
    ...
    def _check_dependencies(
        ...
        for dep in self.dependencies.get(service, []):
            ...
            dep_events = [
                event for event in self.event_timeline
                if (event["service"] == dep and
        ...
        if any(e["type"] == "recent_changes" for e in evidence):
            ...
            if changes["changes"]:
                ...
                if first_change["change_type"] == "deployment":
                    return RootCauseType.DEPLOYMENT, f"Recent deployment: {first_change['description']}"
                elif first_change["change_type"] == "configuration":
                    return RootCauseType.CONFIGURATION, f"Configuration change: {first_change['description']}"
