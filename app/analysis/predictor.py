...
class PredictiveAnalytics:
    ...
    def __init__(self, history_size: int = 1000):
        ...
        # Time series data
        self.time_series: dict[str, deque] = defaultdict(lambda: deque(maxlen=history_size))
    ...
    def add_observation(self, metric_name: str, value: float, timestamp: float | None = None):
        """Add observation to time series"""
        self.time_series[metric_name].append(
            {"value": value, "timestamp": timestamp or time.time()}
        )

    def forecast_load(self, metric_name: str, horizon_minutes: int = 30) -> Prediction | None:
        """
        ...
        confidence_interval = (max(0, forecast - 2 * stdev), forecast + 2 * stdev)

        # Generate recommendations
        recommendations = self._generate_load_recommendations(forecast, values[-1], trend_direction)
    ...
    def predict_failure(self, error_rate_metric: str, threshold: float = 0.1) -> Prediction | None:
    ...
    def predict_resource_exhaustion(
        self, resource_metric: str, capacity: float, critical_threshold: float = 0.9
    ) -> Prediction | None:
    ...
    def _generate_load_recommendations(
        self, predicted_load: float, current_load: float, trend: str
    ) -> list[str]:
    ...
    def get_recent_predictions(
        self, prediction_type: PredictionType | None = None, limit: int = 100
    ) -> list[Prediction]:
