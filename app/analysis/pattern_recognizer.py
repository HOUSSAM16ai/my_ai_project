...
class PatternRecognizer:
    ...
    def analyze_traffic_pattern(self, metric_name: str, value: float) -> list[Pattern]:
        """Analyze traffic patterns"""
        self.stats["total_checks"] += 1

        # Add to history
        self.metric_history[metric_name].append({"value": value, "timestamp": time.time()})
    ...
    def _detect_spike(self, metric_name: str, current_value: float) -> Pattern | None:
    ...
    def _detect_drop(self, metric_name: str, current_value: float) -> Pattern | None:
        """Detect traffic drop"""
        history = [d["value"] for d in self.metric_history[metric_name]]
    ...
    def _detect_periodic_pattern(self, metric_name: str) -> Pattern | None:
        ...
        numerator = sum(
            (recent[i] - mean_recent) * (previous[i] - mean_previous) for i in range(len(recent))
        )
    ...
    def detect_error_pattern(self, error_type: str, timestamp: float) -> Pattern | None:
        """Detect error clustering pattern"""
        metric_name = f"error_{error_type}"

        self.metric_history[metric_name].append({"value": 1, "timestamp": timestamp})

        # Check for error clustering (many errors in short time)
        recent_errors = [
            d
            for d in self.metric_history[metric_name]
    ...
    def detect_security_pattern(
        self, ip_address: str, event_type: str, timestamp: float
    ) -> Pattern | None:
        """Detect security attack patterns"""
        metric_name = f"security_{ip_address}"

        self.metric_history[metric_name].append(
            {"value": 1, "timestamp": timestamp, "event_type": event_type}
        )
    ...
    def get_recent_patterns(
        self, pattern_type: PatternType | None = None, limit: int = 100
    ) -> list[Pattern]:
