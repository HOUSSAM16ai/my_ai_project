...
def create_success_response(data, pagination=None, message="Operation successful"):
    ...
    return {
        "status": "success", "data": response_data, "message": message,
        "timestamp": datetime.datetime.utcnow().isoformat()
    }

# --- User Endpoints ---
@api_v1_blueprint.router.get("/users", status_code=200)
async def get_users(page: int = 1, per_page: int = 20, sort_by: str = "created_at", sort_order: str = "asc", email: str = None):
    users_data = []
    if email:
    ...
    pagination_data = {
        "page": page, "per_page": per_page, "total_items": len(users_data),
...
async def get_tasks(mission_id: int = None):
    tasks_data = []
    if mission_id:
...
async def health():
    return {
        "status": "success", "message": "API v1 is healthy",
        "data": {
            "status": "healthy", "version": "v3.0-hyper", "database": "connected"
        },
        "timestamp": datetime.datetime.utcnow().isoformat()
    }
