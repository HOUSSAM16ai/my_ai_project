...
We prioritize "Privacy by Design." No Personally Identifiable Information (PII) is collected from minors; participants use random IDs. Data collected includes anonymized audit logs (prompts/responses) and aggregate demographic ranges. All data is encrypted at rest (AES-256) and in transit. Raw datasets are accessible only to the Principal Investigator and Data Steward. We commit to open-access publishing of aggregated findings but will never release raw data that could re-identify a vulnerable user.
