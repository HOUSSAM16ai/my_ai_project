...
def test_llm_client_extreme_mode():
    ...
    try:
        ...
        if llm_file.exists():
            ...
            test_result(
                "Extreme Mode Environment Check",
                has_extreme_check,
                "(checks env variable)"
            )
...
def test_admin_ai_extreme_support():
    ...
    try:
        ...
        test_result(
            "MAX_QUESTION_LENGTH Increased",
            max_length >= 100000,
            f"(value: {max_length:,} chars)"
        )
        
        # Check max response tokens
        max_tokens = getattr(admin_ai_service, 'MAX_RESPONSE_TOKENS', 0)
        test_result(
            "MAX_RESPONSE_TOKENS Increased",
            max_tokens >= 32000,
            f"(value: {max_tokens:,} tokens)"
        )
        ...
        if admin_file.exists():
            ...
            test_result(
                "Extreme Mode Guidance in Errors",
                has_extreme_guidance,
                "(suggests activation)"
            )
...
def test_planner_extreme_limits():
    ...
    try:
        ...
        if planner_file.exists():
            ...
            test_result(
                "MAX_TASKS_GLOBAL Increased to 800",
                has_800_tasks,
                "(was 550)"
            )
        else:
...
def test_env_example_documentation():
    ...
    try:
        ...
        if env_file.exists():
            ...
            test_result(
                "Extreme Threshold Documentation",
                has_threshold_docs,
                "(20K chars threshold)"
            )
...
def test_documentation_files():
    ...
    try:
        ...
        test_result(
            "Extreme Mode Comprehensive Guide",
            guide_file.exists(),
            f"({guide_file.name})"
        )
        ...
        if guide_file.exists():
            ...
            test_result(
                "Guide Has Tech Giant Comparison",
                has_comparison,
                "(shows superiority)"
            )
