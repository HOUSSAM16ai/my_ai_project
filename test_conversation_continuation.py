...
def test_conversation_continuation():
    ...
    try:
        ...
        with app.app_context():
            ...
            if not user:
                user = User(full_name="Test User", email="test@test.com", is_admin=True)
            ...
            conversation = service.create_conversation(
                user=user, title="Test Conversation for Continuation", conversation_type="test"
            )
            ...
            messages = (
                AdminMessage.query.filter_by(conversation_id=conversation.id)
            ...
            other_user = User(full_name="Other User", email="other@test.com", is_admin=True)
            ...
            export_result = service.export_conversation(
                conversation_id=conversation.id, format="markdown"
            )
            ...
            # Test HTML export
            export_result = service.export_conversation(
                conversation_id=conversation.id, format="html"
            )
            ...
            success = service.update_conversation_title(
                conversation_id=conversation.id, new_title="Updated Test Conversation"
            )
            ...
            success = service.update_conversation_title(
                conversation_id=conversation.id, auto_generate=True
            )
            ...
            print("  • Continuation: ✅ Working")
            print("  • Security: ✅ Validated")
            print("  • Smart Summary: ✅ Working")
            print("  • Export (3 formats): ✅ Working")
            print("  • Title Management: ✅ Working")
