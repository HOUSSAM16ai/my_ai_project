...
def list_tables():
    ...
    with app.app_context():
        ...
        try:
            ...
            print_section(f"📈 الإحصائيات العامة / General Statistics")
            ...
            expected_tables = [t for t in table_categories.keys() if table_categories[t][0] != 'system']
            missing_tables = [t for t in expected_tables if t not in all_tables]
            
            if missing_tables:
                print_section(f"⚠️ جداول مفقودة / Missing Tables")
                ...
                print_section(f"✅ التحقق من الجداول / Table Verification")
            ...
            print(f"{BOLD}✅ الجداول الموجودة / Present Tables:{E} {G}{len(all_tables) - len(missing_tables)}/{len(expected_tables) + len(system_tables)}{E}")
            if missing_tables:
