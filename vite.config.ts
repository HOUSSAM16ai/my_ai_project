...
export default defineConfig(({ mode }) => {
  ...
  const isSWEnabled = env.VITE_ENABLE_SW === 'true'
      ...
      VitePWA({
        ...
        workbox: {
          ...
          runtimeCaching: [
            {
              urlPattern: ({ url }) => {
                return url.pathname.includes('chart-vendor') ||
                       url.pathname.includes('ai-vendor') ||
                       url.pathname.includes('editor-vendor');
              },
              handler: 'CacheFirst' as const,
              options: {
                cacheName: 'large-vendor-chunks-cache',
                expiration: {
                  maxEntries: 10,
