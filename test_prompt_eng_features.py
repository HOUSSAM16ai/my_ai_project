...
def test_prompt_injection_detection():
    ...
    for prompt in safe_prompts:
        ...
        status = "✅" if not result["is_malicious"] else f"⚠️ False positive"
        print(f"    {status} Risk: {result['risk_level']}/10")
    
    print("  Malicious prompts:")
    for prompt in malicious_prompts:
        result = service.detect_prompt_injection(prompt)
        status = "✅" if result["is_malicious"] else f"❌ Missed"
        print(f"    {status} Risk: {result['risk_level']}/10 - {len(result['detected_patterns'])} patterns")
    print()
...
def test_content_sanitization():
    ...
    for dirty, expected in tests:
        ...
        status = "✅" if expected in clean else f"⚠️"
...
def test_risk_classification():
    ...
    for prompt, desc, expected_category in tests:
        ...
        status = "✅" if result["category"] == expected_category else f"⚠️ (got {result['category']})"
