...
This document outlines a detailed, per-file plan for safely decoupling legacy framework dependencies. The core strategy is to introduce compatibility wrappers and use dependency injection to progressively migrate components without breaking the existing application.
...
1.  **File:** `app/core/database.py`
    *   **Action:** Create a standalone SQLAlchemy session factory that does not depend on global framework context objects. This will allow services to acquire database sessions directly.
    *   **Acceptance Criteria:** The session factory can be used in a simple Python script to connect to the database and perform a query without a web app context.

2.  **File:** `app/core/compat.py` (or similar)
    *   **Action:** Create a compatibility module that provides a shim for the legacy app context. This shim will emulate the necessary attributes (like `logger` and `config`) so that legacy services can run in a non-framework context during the transition.
    *   **Acceptance Criteria:** A service that uses context-based logging can be imported and called from a test without requiring any global app context setup.

---

### Step B: Refactor a High-Risk Service with DI

**Goal:** Migrate a single, high-risk service to use the new dependency injection pattern instead of relying on a global framework context.

1.  **Target File:** `app/services/history_service.py` (Example)
    *   **Current Behavior:** Uses global context objects and framework-bound authentication helpers.
    *   **Action:**
        ...
        3.  Update the corresponding HTTP routes to call the refactored service methods, passing the required dependencies.
    *   **Acceptance Criteria:** The service's unit tests pass without a framework app context. The live application continues to function as before.
...
1.  **File:** `tests/transcendent/test_db_session_invariants.py`
    *   **Action:** Write a test that acquires a database session using both the old (`db.session`) and new (`get_db_session`) methods and asserts that they are part of the same underlying transaction when used within a legacy request.
    *   **Acceptance Criteria:** The test passes, proving that both systems can coexist safely.

2.  **File:** `tests/transcendent/test_no_framework_imports.py`
    *   **Action:** Write a test that programmatically scans all files in `app/services/` and fails if it finds any direct imports from the legacy web framework.
...
2.  **Details:**
    *   Attach the `reports/framework_references_classified.json` and `migration/STEP1_migration_map.md` files to the PR for context.
...
4.  **Rollback Instructions:**
    ...
    *   **Database:** If a migration needs to be rolled back, use `python -m alembic downgrade -1`. If the downgrade path is not clean, restore the database from the snapshot taken before the migration.
