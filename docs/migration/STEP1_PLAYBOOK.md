...
1.  **File:** `app/core/database.py`
    *   **Action:** Create a standalone SQLAlchemy session factory that does not depend on legacy framework globals. This will allow services to acquire database sessions directly.
    *   **Acceptance Criteria:** The session factory can be used in a simple Python script to connect to the database and perform a query without a legacy app context.

2.  **File:** `app/core/compat.py` (or similar)
    *   **Action:** Create a compatibility module that provides a shim for `current_app`. This shim will emulate the necessary attributes (like `logger` and `config`) so that legacy services can run in a non-legacy context during the transition.
    *   **Acceptance Criteria:** A service that uses `current_app.logger` can be imported and called from a test without requiring legacy app context setup.

---

### Step B: Refactor a High-Risk Service with DI

**Goal:** Migrate a single, high-risk service to use the new dependency injection pattern instead of relying on legacy global context.

1.  **Target File:** `app/services/history_service.py` (Example)
    *   **Current Behavior:** Uses legacy context imports such as `current_app` and `current_user`.
    *   **Action:**
        ...
        3.  Update the corresponding legacy routes to call the refactored service methods, passing the required dependencies.
    *   **Acceptance Criteria:** The service's unit tests pass without a legacy app context. The live application continues to function as before.
...
2.  **File:** `tests/transcendent/test_no_legacy_imports.py`
    *   **Action:** Write a test that programmatically scans all files in `app/services/` and fails if it finds any direct imports from legacy framework modules (e.g., `from legacy_context import current_app`).
...
2.  **Details:**
    *   Attach the legacy reference reports and `migration/STEP1_migration_map.md` files to the PR for context.
