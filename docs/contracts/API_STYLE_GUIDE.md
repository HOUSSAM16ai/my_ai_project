...
## 🔌 توحيد دلالات الاستهلاك في AsyncAPI عبر بروتوكولات متعددة

عند تصميم قناة واحدة تُكشف عبر بروتوكولين مختلفين (مثل Kafka وWebSocket) يجب الانتباه
إلى أن **دلالة الاستهلاك** تختلف جذرياً: Kafka غالباً **Queue / Consumer Group** بينما
WebSocket غالباً **Broadcast** إلى جميع العملاء المتصلين. لتفادي التعارض الدلالي:

### ✅ القرار الأساسي
* **إذا كانت دلالات الاستهلاك مختلفة** ➜ **لا تستخدم قناة واحدة**. عرّف قناتين منفصلتين
  بعناوين مختلفة حتى لو كانت الرسائل نفسها.
* **إذا كانت الدلالات متطابقة فعلياً** (مثل بث لكل المشتركين في كلا البروتوكولين) ➜ يمكن
  استخدام قناة واحدة مع توثيق صريح للدلالة.

### 🔍 كيف تعالج AsyncAPI التعارض؟
* **الـ bindings خاصة بالبروتوكول**: وجود `groupId` في Kafka **لا يُفترض** أن ينطبق على
  WebSocket. غياب `groupId` في WebSocket طبيعي ومسموح، لكنه يعني أن القناة **لا تتبنى
  دلالة المستهلك المشترك** في هذا البروتوكول.
* **المواصفة لا تُوحِّد الدلالات تلقائياً**: يجب توثيق المقصود بوضوح في `description` أو
  عبر امتداد مخصّص (`x-consumption-model`).

### 🧭 نمط موصى به (قناتان منفصلتان)
```yaml
channels:
  account.created.v1:
    description: "Kafka: Queue semantics عبر Consumer Group."
    servers:
      - production
    subscribe:
      bindings:
  ...
  account.created.v1.broadcast:
    description: "WebSocket: Broadcast لكل العملاء المتصلين."
    servers:
      - websocket-dashboard
    subscribe:
      bindings:
        ws:
          method: GET
        x-consumption-model: broadcast
```

### 🧩 بديل عند الحاجة لقناة واحدة فقط
إذا كان فرض قناة واحدة إلزامياً، يجب أن تكون **الدلالة موحّدة** (مثلاً Broadcast في كلا
الطرفين)، مع توثيق صريح:

```yaml
channels:
  account.created.v1:
    description: "Broadcast عبر Kafka (لا تُستخدم مجموعات مستهلكين) وعبر WebSocket."
    subscribe:
      bindings:
        x-consumption-model: broadcast
```

> **قاعدة ذهبية:** لا تجعل عنوان قناة واحد يحمل دلالتين متعارضتين؛ ذلك يخلق عقداً
> غير صالح دلالياً ويؤدي لالتباس المستهلكين.
