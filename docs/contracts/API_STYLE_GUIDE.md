...
### 🧬 تعدد الأشكال (Polymorphism) في OpenAPI 3.1

عند تصميم مخططات متعددة الأشكال (مثل `Dog | Cat`) في OpenAPI 3.1 باستخدام JSON Schema 2020-12، يجب مراعاة ما يلي لضمان التوافق مع مولدات الأكواد والمحللات المتدفقة عالية الأداء:

#### ✅ القرار المعماري
1. **استخدم `discriminator` صراحةً** عند وجود مسارات متعددة محتملة لفكّ التسلسل (Deserialize).
2. **لا تعتمد فقط على `const` داخل `oneOf/anyOf`** كمحدد وحيد للنوع، لأن كثيرًا من المولدات لا تُنفّذ محرك JSON Schema كامل ولا تستطيع الحسم المبكر أثناء التحليل المتدفق.

#### لماذا `const` وحده غير كافٍ في المحللات المتدفقة؟
- المحلل المتدفق يحتاج تحديد النوع قبل قراءة الكائن كاملًا، بينما خصائص `const` قد تأتي متأخرة في الحمولة.
- بعض المولدات تعتمد اختيارًا جزئيًا أو ترتيبًا ثابتًا للبدائل، ما يؤدي إلى فك تسلسل خاطئ أو غير مستقر.

#### ✅ صرامة الخصائص: `unevaluatedProperties` في OAS 3.1
في OpenAPI 3.1:
- استخدم `unevaluatedProperties: false` لضمان رفض الخصائص غير المُقيّمة بعد اكتمال تقييم جميع التركيبات (`oneOf`/`allOf`/`anyOf`).
- هذا أكثر عدالة من `additionalProperties: false` في OAS 3.0، لأنه يسمح للخصائص المُعرّفة في أي فرع بالمرور قبل الحسم النهائي.

#### ✅ نمط موصى به (قابل للنسخ)
```yaml
Dog:
  type: object
...
> **ملاحظة:** رغم أن OpenAPI 3.1 يدعم JSON Schema كاملًا، يبقى `discriminator` هو الخيار الأكثر موثوقية لتوليد الأكواد وفكّ التسلسل في البيئات عالية الأداء.
