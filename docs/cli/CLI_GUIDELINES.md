...
- **No legacy framework imports**: CLI handlers in `app/cli_handlers` must not import `current_app` or any other legacy framework context variables. This ensures that the CLI is decoupled from the web application.
